...
class UpStash {
     ...
     * @param {Function<Promise<string>>} callback - Callback function to fetch the value
     * @returns {Promise<string>} The requested value
     */
    async hitOrMiss(key, callback) {
        const cachedValue = await this.getCache(key);
        if (cachedValue) {
            return cachedValue;
        } else {
